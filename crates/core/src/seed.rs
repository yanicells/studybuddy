//! Sample nested library for local testing.
...
const COURSE: &str = "CSCI 50.01";
const LECTURE: &str = "Hardware Lecture";

const ARCH_ORG: &str = r#"What is computer architecture?
...
const STRUCT_FN: &str = r#"How are modern computers arranged despite millions of components?
...
/// Insert the CSCI 50.01 sample if it is not already in the library.
pub fn sample_if_missing(store: &Store) -> Result<bool> {
    if store
        .list_folders()?
        .iter()
        .any(|folder| folder.name == COURSE)
    {
        return Ok(false);
    }

    let course = store.create_folder(None, COURSE)?;
    let lecture = store.create_folder(Some(course.id), LECTURE)?;

    let arch = store.create_deck(Some(lecture.id), "Architecture vs Organization")?;
    store.import_cards(arch.id, &import::parse(ARCH_ORG))?;

    let structure = store.create_deck(Some(lecture.id), "Structure and Function")?;
    store.import_cards(structure.id, &import::parse(STRUCT_FN))?;
...
mod tests {
    ...
    #[test]
    fn sample_has_enough_main_cards() {
        let arch = parse(ARCH_ORG);
        let structure = parse(STRUCT_FN);
        assert!(arch.len() >= 15, "arch {}", arch.len());
        assert!(structure.len() >= 25, "structure {}", structure.len());
        assert!(arch.len() + structure.len() >= 40);
        assert!(arch.iter().all(|c| !c.highlights.is_empty()));
        assert!(structure.iter().all(|c| !c.highlights.is_empty()));
    }

    #[test]
    fn seeds_nested_folders_once() {
        let store = Store::open_in_memory().unwrap();
        assert!(sample_if_missing(&store).unwrap());
        assert!(!sample_if_missing(&store).unwrap());
        let folders = store.list_folders().unwrap();
        assert!(folders.iter().any(|f| f.name == COURSE && f.parent_id.is_none()));
        let course = folders.iter().find(|f| f.name == COURSE).unwrap();
        let lecture = folders
            .iter()
            .find(|f| f.name == LECTURE && f.parent_id == Some(course.id))
            .unwrap();
        let decks = store.list_decks().unwrap();
        assert_eq!(
            decks
                .iter()
                .filter(|d| d.folder_id == Some(lecture.id))
                .count(),
            2
        );
    }
